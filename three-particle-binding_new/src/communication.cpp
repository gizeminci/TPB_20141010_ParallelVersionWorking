...
void mpi_who_has()
{
  ...
  for (pnode = 0; pnode < n_nodes; pnode++) {
    COMM_TRACE(fprintf(stderr, "node %d reports %d particles\n",
		       pnode, sizes[pnode]));
//printf("node %d reports %d particles\n", pnode, sizes[pnode]);
    if (pnode == this_node) {
      for (c = 0; c < local_cells.n; c++) {
	cell = local_cells.cell[c];
	for (i = 0; i < cell->n; i++)
	  particle_node[cell->part[i].p.identity] = this_node;
//printf("node %d reports particle %d\n", pnode, cell->part[i].p.identity);
      }
    }
...
//printf("%d: processing %s %d %d...\n", this_node, names[request[0]], request[1], request[2]);
